...
export const SETTING_CONSTRAINTS = {
  ...
  scrollOffset: { min: -500, max: 500, default: 0 },
}
...
export class MarkmapSettingTab extends SettingTab {
  ...
  onshow(): void {
    ...
    this.addSetting(item => {
      item.addName('滚动偏移量')
      item.addDescription('点击节点跳转时，标题距离窗口顶部的像素距离。可以为负数。')
      item.addInput('number', (input) => {
        const config = SETTING_CONSTRAINTS.scrollOffset
        input.value = this.settings.get('scrollOffset').toString()
        input.min = config.min.toString()
        input.max = config.max.toString()
        input.onchange = () => this.settings.set('scrollOffset', parseInt(input.value) || config.default)
      })
    })
